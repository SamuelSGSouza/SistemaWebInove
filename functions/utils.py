...
def verifica_base_dfv(request, extensao, arquivo_original,caminho_final, df:pd.DataFrame, sistema:str) -> list:
    if sistema == "oi":
        ...
        COLUNAS_DFV = ["CNPJ", "UF", "ESTACAO", "CELULA", "TIPO_VIABILIDADE", "Cidade", "LOCALIDADE", "Razao_Social", "ENDERECO", "FACHADA", "COMPLEMENTO", "CEP", "Bairro", "CNAE_Descricao", "CREDITO_PREAPROVADO", "MOTIVO_NEGACAO", "LETRA_MOTIVO_NEGACAO", "FLAG_POSSE_VELOX", "contato1", "contato2", "contato3", "contato4", "TERMINAL_CONECTADO", "Nome_Representante", "LINHA_PRODUTO", "FLAG_PROSPECT", "FLG_PREDIO", "CNAE_CODIGO", "CPF_REPRESENTANTE", "POSSE_FIBRA_CPF", "TIPO_LOGRADOURO_INST_REP", "NOME_LOGRADOURO_INST_REP", "NUMERO_INSTALACAO_REP", "TIPO_COMPLEMENTO_INSTALACAO_REP", "NUMERO_COMPLEMENTO_1_INST_REP", "NUMERO_COMPLEMENTO_2_INST_REP", "NUMERO_COMPLEMENTO_3_INST_REP", "BAIRRO_INSTALACAO_REP", "MUNICIPIO_INSTALACAO_REP", "ESTADO_INSTALACAO_REP", "CEP_INSTALACAO_REP", "MOTIVO", "SUBMOTIVO", "COD_ENCERRAMENTO", "DESC_FINAL", "ESTADO", "COD_VIABILIDADE", "APROVACAO_RECENTE", "CNPJ_NOVO_NO_MAILING", "ORIGEM", "SOCIO_COM_FIBRA_NO_ENDERECO", "PROPENSAO_IP", "RETIRADA_DADOS", "PROPENSAO_CLOUD", "PLANTA_VOZ_AVANCADA", "FLG_ENRIQUECIMENTO", "CPF_SOCIO", "CONTATO_SOCIO", "CLASSIFICACAO"]
...
def verificador_fase_1():
    ...
    status_processo = StatusProcesso.objects.filter()[0]
    ...
    for estado in estados:
        ...
        if hoje.day != data.day or hoje.month != data.month:
            ...
            salva_log(f"Erro ao Tratar Base da Receita: Arquivo {file} não foi criado hoje.", sistema="geral")
            status_processo.status_fase_1 = "ERRO"
            status_processo.progresso_fase_1 = 100
            status_processo.fase_1_color = "danger"
            status_processo.save()
            return False
        
        #verificar se todos os estados possuem as mesmas colunas
        df = pd.read_csv(filepath, sep=";")
        if df.columns.tolist() != colunas_esperadas:
            salva_log(f"Erro ao Tratar Base da Receita: Arquivo {file} não possui as colunas esperadas", sistema="geral")
            status_processo.status_fase_1 = "ERRO"
            status_processo.progresso_fase_1 = 100
            status_processo.fase_1_color = "danger"
            status_processo.save()
        ...
        if len(df["cnpj"].tolist()) != len(df["cnpj"].unique().tolist()):
            salva_log(f"Erro ao Tratar Base da Receita: Arquivo {file} possui cnpjs repetidos", sistema="geral")
            status_processo.status_fase_1 = "ERRO"
            status_processo.progresso_fase_1 = 100
            status_processo.fase_1_color = "danger"
            status_processo.save()
        ...
        if len(df_repetidos.index) > 1:
            salva_log(f"Erro ao Tratar Base da Receita: Arquivo {file} possui cnpjs repetidos com outro arquivo", sistema="geral")
            status_processo.status_fase_1 = "ERRO"
            status_processo.progresso_fase_1 = 100
            status_processo.fase_1_color = "danger"
            status_processo.save()
    ...
    #verificar se há telefones repetidos